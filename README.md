...
1. Can a model built on real-time data separate recession months from expansion months? — **Yes, strongly.** ROC-AUC 0.965 at horizon 0.
2. Does it fire meaningfully before NBER's official call? — **Yes** — flagged all 3 in-sample recessions ahead of the announcement, averaging ~5 months lead.
...
- **Ensemble underperforms the single model** (see Results) — the stacked ensemble is included for completeness, not because it's the recommended model. LightGBM alone is the better choice as it stands.
...
- **A known inconsistency in the setup script:** `data_pull.py` reads the FRED key from the environment variable `alfred_api_key`, but its own error message tells the user to set `FRED_API_KEY` — using the name in the error message will fail silently into the "key not found" branch. Use `alfred_api_key` in your `.env`.
